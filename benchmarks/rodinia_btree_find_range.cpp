...
#include "rodinia_src/btree/kernel_gpu_cuda_wrapper_2.h"
...
verify_results(        
            ...
            int *start,
            int *end,
            int *dhopper_recstart,
            int *dhopper_reclength) {
    ...
    int *referenceRecstart  = (int *) malloc(count * sizeof(int));
    int *referenceReclength = (int *) malloc(count * sizeof(int));
    // OUTPUT: ans CPU initialization
    for (int i = 0; i < count; i++) {
        referenceRecstart[i]  = 0;
        referenceReclength[i] = 0;
    }
    ...
    bool success = true;
    for (int i = 0; i < count; ++i) {
        if (dhopper_recstart[i] != referenceRecstart[i] || dhopper_reclength[i] != referenceReclength[i]) {
            std::cout << "Error at index: " << i << " ";
            std::cout << "Expected: Start: " << referenceRecstart[i] << " Length: " << referenceReclength[i] << " | "
                        << "Start: " << dhopper_recstart[i] << " Length: " << dhopper_reclength[i] << std::endl;
            success = false;
            break;
        }
    }

    free(referenceRecstart);
    free(referenceReclength);
...
DEVICE_HOPPER_MAIN(int argc, char* argv[]) {
    ...
    // TODO the problem sizes are zero based in contrast to find k
    std::string path_to_input_file;
    int count = 0;
    int r_size = 0;
    if (problem_size == 2) {
        count = 4000000;
        path_to_input_file = REPOSITORY_PATH + "/library/benchmarks/input_data/rodinia/b+tree/4000k.txt";
    } else if (problem_size == 1) {
        count  = 2500000;
        r_size = 300000;
        path_to_input_file = REPOSITORY_PATH + "/library/benchmarks/input_data/rodinia/b+tree/2500k.txt";
    } else if (problem_size == 0) {
        count  = 1350000;
        r_size = 300000;
        path_to_input_file = REPOSITORY_PATH + "/library/benchmarks/input_data/rodinia/b+tree/1350k.txt";
    } else {
    ...
    long *lastKnode = (long *) device_hopper::malloc(count, sizeof(long));
    // INPUT: offset CPU initialization
    memset(lastKnode, 0, count * sizeof(long));

    // INPUT: offset_2 CPU allocation
    long *offset_2 = (long *) device_hopper::malloc(count, sizeof(long));
    // INPUT: offset CPU initialization
    memset(offset_2, 0, count * sizeof(long));

    // INPUT: start, end CPU allocation
    int *start = (int *) device_hopper::malloc(count, sizeof(int));
    int *end = (int *) device_hopper::malloc(count, sizeof(int));
    // INPUT: start, end CPU initialization
    for (int i = 0; i < count; i++) {
        start[i] = (rand() / (float) RAND_MAX) * size;
        end[i] = start[i] + r_size;
        if (end[i] >= size) {
    ...
    // INPUT: recstart, reclenght CPU allocation
    int *recstart = (int *) device_hopper::malloc(count, sizeof(int));
    int *reclength = (int *) device_hopper::malloc(count, sizeof(int));
    // OUTPUT: ans CPU initialization
    for (int i = 0; i < count; i++) {
        recstart[i] = 0;
        reclength[i] = 0;
    ...
    pf.add_buffer_access_patterns(
        ...
        device_hopper::buf(lastKnode, direction::IN_OUT, data_kind::INTERIM_RESULTS, pattern::SUCCESSIVE_SUBSECTIONS(1)),
        device_hopper::buf(offset_2,  direction::IN_OUT, data_kind::INTERIM_RESULTS, pattern::SUCCESSIVE_SUBSECTIONS(1)),
        device_hopper::buf(start,     direction::IN,  pattern::SUCCESSIVE_SUBSECTIONS(1)),
        device_hopper::buf(end,       direction::IN,  pattern::SUCCESSIVE_SUBSECTIONS(1)),
        device_hopper::buf(recstart,  direction::OUT, pattern::SUCCESSIVE_SUBSECTIONS(1)),
        device_hopper::buf(reclength, direction::OUT, pattern::SUCCESSIVE_SUBSECTIONS(1)));
    // Add scalar kernel parameters
    pf.add_scalar_parameters(maxheight, knodes_elem);
    // Set optional tuning parameters and call run()
    // this would require adapting the source to source translator though
