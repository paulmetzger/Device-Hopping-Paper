...
__kernel void fft1D_512_with_slicing (__global T2 *work, const int gid)
{
  int tid = get_local_id(0);
  //int blockIdx = get_global_id(0) * 8 + tid; // Does not work
  //int blockIdx = (get_global_id(0) >> 6) * 512 + tid;
  //int blockIdx = (get_global_id(0) / 64) * 512 + tid;
  int blockIdx = (gid + get_group_id(0)) * 512 + tid;
...
__kernel void fft1D_512_original (__global T2 *work)
{
  int tid = get_local_id(0);
  int blockIdx = get_group_id(0) * 512 + tid;
  int hi = tid>>3;
...
/*__kernel void ifft1D_512_with_slicing (__global T2 *work)
{
  int i;
  int tid = get_local_id(0);
  //int blockIdx = (get_global_id(0) >> 6) * 512 + tid;
  int blockIdx = 0; //get_group_id(0) * 512 + tid;
  int hi = tid>>3;
  int lo = tid&7;
  T2 data[8];
  __local T smem[8*8*9];
...
__kernel void ifft1D_512_original (__global T2 *work)
{
  int i;
  int tid = get_local_id(0);
  int blockIdx = get_group_id(0) * 512 + tid;
...
__kernel void chk1D_512(__global T2* work, int half_n_cmplx, __global int* fail)
{
